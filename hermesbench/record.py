...
class HyperframesRecorder:
    ...
    def build_armed_session(self):
        ...
        r = self._tmux("new-session", "-d", "-s", self.session, "-x", "200", "-y", "56",
                       "-P", "-F", "#{pane_id}", armed("Runner", runner_cmd))
        pane0 = r.stdout.strip()

        r = self._tmux("split-window", "-h", "-t", pane0, "-l", "45%",
                       "-P", "-F", "#{pane_id}", armed("Metrics", metrics_cmd))
        pane1 = r.stdout.strip()

        r = self._tmux("split-window", "-v", "-t", pane0, "-l", "55%",
                       "-P", "-F", "#{pane_id}", armed("Agent", agent_cmd))
        pane2 = r.stdout.strip()

        r = self._tmux("split-window", "-v", "-t", pane1, "-l", "55%",
                       "-P", "-F", "#{pane_id}", armed("Scoreboard", score_cmd))
        pane3 = r.stdout.strip()

        r = self._tmux("split-window", "-v", "-t", pane2, "-l", "20%",
                       "-P", "-F", "#{pane_id}", armed("Telemetry", telemetry_cmd))
    ...
    def run_headless(self):
        ...
        xterm = subprocess.Popen([
            ...
            "-bg", "#0a0a14", "-fg", "#e0e0e8",
            "-cr", "#0a0a14", "-ms", "#0a0a14",
            "-xrm", "XTerm*cursorBlink: false", "-fa", "Monospace", "-fs", "9",
        ...
        if size_mb > 1.0:
            ...
            print(f"WARNING: Output too small ({size_mb:.1f} MB)")
    ...
    def run(self):
        ...
        if self.attach_mode:
            ...
            print(f"Trigger: touch {self.trigger}")
            print(f"Stop: tmux kill-session -t {self.session}")
            return
